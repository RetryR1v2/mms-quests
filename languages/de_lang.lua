Locales["de_lang"] = {
    PromptName = 'Quest Annehmen',
    BlipNameQuestgeber = 'Questgeber',
    QuestMenuHeader = 'The Heartland Revengers Quests',
    CloseQuestMenu = 'Quest Menü Schließen',
    QuestStart = 'Starte eine Quest',
    QuestShop = 'Quest Gegenstände Eintauschen',
    QuestAbort = 'Quest Abbrechen',
    AlreadyActiveQuest = 'Du hast bereits eine Quest Angenommen',
    QuestAborted = 'Quest Abgebrochen',
    NoActiveQuest = 'Keine Quest Aktiv',
    QuestStarted = 'Quest Gestartet',
    NoInvetorySpace = 'Deine Tasche ist zu Voll',
    YouGot = 'Du Bekommst: ',
    And = ' Und ',
    Back = 'Zurück',
    TokenShopHeader = 'The Heartland Revengers Token Shop',
    CloseTokenShop = 'Token Shop Schließen',
    For = ' Für ',
    BuyForXToken = ' Quest Token kaufen!',
    Description = 'Die Tier Gutscheine können beim Gouverneur Eingelöst werden!',
    NotEnoghToken = 'Nicht genug Quest Token zum Kaufen',
    SuccessfullyBoughtTicket = 'Erfolgreich gekauft',
    CanDoQuestAgain = 'Die Wartezeit für Quests ist Abgelaufen',
    YouAbortedAlready = 'Du hast eine Quest Abgebrochen Bitte Warte: Sekunden ',
    CloseActiveQuestMenu = 'Schließen',
    ActiveQuestHeader = 'The Heartland Revengers Quests',
    MissingItems = 'Oh Nein, Hier fehlt etwas Bitte besorge die folgenen Gegenstände: ',
    BringPlease = 'Bitte bringe: ',
    DeliverItems = 'Gegenstände Übergeben',
    ActiveQuest = 'Aktive Quest: ',
    MissingItems2 = 'Du hast nicht alle benötigten Items.',
    WHText = 'Der Spieler: ',
    WHId = ' Mit der Charakter ID: ',
    WHGot = ' Bekommt von Quest: ',
}